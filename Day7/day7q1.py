...
counts = [list(Counter(x[0]).values()) for x in hands]
...
for i in range(1,8):
    ...
    for x in range(len(hands)):
        if counts[x] == i:
            currConsideration.append([hands[x][0].replace('K', 'B').replace('Q', 'C').replace('J', 'D').replace('T', 'E').replace('9', 'F').replace('8', 'G').replace('7', 'H').replace('6', 'I').replace('5', 'J').replace('4', 'K').replace('3', 'L').replace('2', 'M'), hands[x][1]])
